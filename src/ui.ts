...
import type {
  WireTool, WireApproval, WireAsk, WireCompaction,
  CheckpointMeta, HistoryMessage, TodoItem, MessageState,
} from "./types";
...
// ── message rendering ──
export function addUserMsg(log: HTMLElement, welcome: HTMLElement | null, text: string, scrollCb?: () => void): void {
...
export function ensureMsg(log: HTMLElement, welcome: HTMLElement | null, state: MessageState): HTMLElement {
  if (!state.currentMsg) {
    if (welcome) welcome.style.display = "none";
...
export function appendText(log: HTMLElement, welcome: HTMLElement | null, state: MessageState, t: string, scrollCb?: () => void): void {
...
export function appendReasoning(log: HTMLElement, welcome: HTMLElement | null, state: MessageState, t: string, __: (s: string) => string, scrollCb?: () => void): void {
  const m = ensureMsg(log, welcome, state);
...
export function finalizeMsg(state: MessageState): void {
  if (state.currentMsg) {
...
// ── tool cards ──
export function renderToolDispatch(
  log: HTMLElement, tool: WireTool, toolCards: Record<string, HTMLElement>,
  welcome: HTMLElement | null, scrollCb?: () => void,
): void {
  if (welcome) welcome.style.display = "none";
  ...
  head.innerHTML =
    '<span class="ico spin"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10" stroke-dasharray="60" stroke-dashoffset="20"/></svg></span><span class="name">' +
    escHtml(tool.name) + "</span>" +
    (tool.args ? '<span class="subject">' + escHtml(tool.args.slice(0, 80)) + "</span>" : "") +
    '<span class="grow"></span><span class="chev"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></span>';
...
export function renderToolResult(tool: WireTool, toolCards: Record<string, HTMLElement>, scrollCb?: () => void): void {
  const card = toolCards[tool.id];
  if (!card) return;
  const ico = card.querySelector(".ico") as HTMLElement | null;
  if (ico) {
    ico.className = "ico";
...
export function renderToolProgress(tool: WireTool, toolCards: Record<string, HTMLElement>, scrollCb?: () => void): void {
  const card = toolCards[tool.id];
  if (!card) return;
  const body = card.querySelector(".card-body") as HTMLElement | null;
  if (!body) return;
...
// ── approval ──
function approvalSessionRule(a: WireApproval): string {
...
function approvalPersistentRule(a: WireApproval): string {
  if (a.tool === "bash" && a.subject) return "Bash(" + String(a.subject).trim() + ")";
...
export function showApproval(log: HTMLElement, a: WireApproval, __: (s: string) => string, scrollCb?: () => void): void {
  ...
  if (hasPrefix) {
    ...
    actions.push('<button class="approval__btn approval__btn--allow" data-allow="true" data-session="true"><span class="approval__key">A</span> ' + escHtml(__("session") + " " + approvalSessionRule(a)) + "</button>");
    actions.push('<button class="approval__btn approval__btn--allow" data-allow="true" data-session="true" data-persist="true"><span class="approval__key">P</span> ' + escHtml(__("persist_tool") + " " + approvalPersistentRule(a)) + "</button>");
  }
  ...
  const resolve = (payload: Record<string, unknown>) => {
    post("/approve", Object.assign({id: a.id}, payload));
    d.remove();
    document.removeEventListener("keydown", onkey);
  };
  d.querySelectorAll(".approval__btn").forEach(btn => {
    (btn as HTMLElement).onclick = () => resolve({
      ...
      scope: (btn as HTMLElement).dataset.scope || "",
    });
  });
...
// ── ask ──
export function showAsk(log: HTMLElement, ask: WireAsk, __: (s: string) => string, scrollCb?: () => void): void {
  const d = el("div", "ask");
  ask.questions.forEach(q => {
    ...
    q.options.forEach((o, i) => {
      ...
      opt.onclick = () => {
        ...
        else { selected.clear(); selected.add(i); opts.querySelectorAll(".ask__opt").forEach((o, j) => (o as HTMLElement).classList.toggle("ask__opt--selected", j === i)); }
      };
    ...
    submit.onclick = () => {
      const answers = ask.questions.map(qq => ({
        questionId: qq.id,
...
// ── compaction ──
export function showCompaction(log: HTMLElement, c: WireCompaction, __: (s: string) => string, scrollCb?: () => void): void {
  ...
  if (c.summary) {
    ...
    head.appendChild(el("span", "", String(c.messages) + " " + __("messages")));
...
export function renderHistoryMessages(
  log: HTMLElement, welcome: HTMLElement | null, state: MessageState,
  ms: HistoryMessage[], toolCards: Record<string, HTMLElement>,
  __: (s: string) => string, scrollCb?: () => void,
): void {
...
export function openStats(
  sessionCount: number, cumulativeTokens: number, cumulativeCost: number,
  cumulativeCacheHit: number, cumulativeCacheMiss: number,
): void {
  const modal = document.getElementById("stats-modal") as HTMLElement | null;
  if (!modal) return;
  getJSON<Record<string, any>>("/status").then(s => {
    const getE = (id: string) => document.getElementById(id) as HTMLElement | null;
    getE("stats-model")!.textContent = s.label || "-";
    getE("stats-sessions")!.textContent = String(sessionCount || "0");
    const total = cumulativeTokens || s.used || 0;
    getE("stats-total-tokens")!.textContent = total >= 1000 ? fmtTok(total) : String(total);
    ...
    getE("stats-cache-rate")!.textContent = rate;
    const cost = cumulativeCost || s.lastUsage?.cost || 0;
    getE("stats-total-cost")!.textContent = cost > 0 ? fmtMoney(cost, s.lastUsage?.currency) : "-";
    getE("stats-balance")!.textContent = s.balance?.display || "-";
    if (s.window) {
      const pct = Math.min(100, Math.round(s.used / s.window * 100));
      getE("stats-ctx-fill")!.style.width = pct + "%";
      getE("stats-ctx-used")!.textContent = fmtTok(s.used) + " tok";
      getE("stats-ctx-window")!.textContent = fmtTok(s.window) + " tok";
    }
  }).catch(() => {});
